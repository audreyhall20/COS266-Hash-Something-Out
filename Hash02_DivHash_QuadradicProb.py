...
# this method uses division hashing paired with quadratic probing. 
...
def hashFunction(key, tableSize):
    sumChars = 0
    for char in key:
        sumChars += ord(char)
...
def add_quadraticProbing(key, hashTableArr, tableSize):
    ...
    startIndex = hashFunction(key, tableSize)
    collisions = 0 
...
def main():
    ...
    print(f"\nStatistics tracking attempt 2: Division hash and quadratic probing")
